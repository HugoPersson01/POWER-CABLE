...
def get_current_force():
    return (motor.axis1.motor.current_control.Iq_measured + 1.7)/ kg_to_current
...
def calibrated_kg_to_current(inmatatKg):
    new_kg_to_current = ((inmatatKg*kg_to_current) -1.7)
    return new_kg_to_current

def convertCurrentToKg():
    return ((motor.axis1.motor.config.current_lim + 1.7)/kg_to_current)
